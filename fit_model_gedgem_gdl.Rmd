...
#grid <- st_read("Grid/grid.shp") %>% st_transform(crs = st_crs(pays))
load("grid.rdata")
>>>>>>> 83c3f30226831dc8061c22df36c44a2a4efc119f
# focus Golfe du Lion
...
=======
cov_bathy <- grid$bathy[ind_effort!=0]
#grid %>% ggplot() + geom_sf(aes(fill = bathy))

data <- unmarkedFrameOccuMulti(y = y, 
                               siteCovs = data.frame(bathy = cov_bathy), 
                               obsCovs = det_covs) #,siteCovs=occ_covs,obsCovs=det_covs)
>>>>>>> 83c3f30226831dc8061c22df36c44a2a4efc119f
```
...
occFormulas <- c('~1','~1','~1') 
=======
occFormulas <- c('~bathy','~bathy','~bathy') 
>>>>>>> 83c3f30226831dc8061c22df36c44a2a4efc119f
...
=======
logit_p1 <- fit@opt$par[7] + fit@opt$par[8] * grid_p
logit_p2 <- fit@opt$par[9] + fit@opt$par[10] * grid_p
>>>>>>> 83c3f30226831dc8061c22df36c44a2a4efc119f
...
=======
  for(i in 1:nsite) {
      log(prop[i, 1]) <- alpha[1] + alpha[2] * bathy[i]
      log(prop[i, 2]) <- alpha[3] + alpha[4] * bathy[i]
      log(prop[i, 3]) <- alpha[5] + alpha[6] * bathy[i]
  }
...
=======
             eff = st_effort,
             bathy = (cov_bathy - mean(cov_bathy)) / sd(cov_bathy))
>>>>>>> 83c3f30226831dc8061c22df36c44a2a4efc119f
```
...
=======
  ...
  for (ii in 1:6){alpha[ii] ~ dnorm(0,1)}
...
=======
mbathy <- mean(cov_bathy)
sdbathy <- sd(cov_bathy)
stbathy <- (cov_bathy - mbathy)/sdbathy
library(mgcv)
...
<!--   for (i in 1:1) { b[i] ~ dnorm(0,0.019) } -->
<!--   ## prior for s(cov_bathy)...  -->
<!--   K1 <- S1[1:9,1:9] * lambda[1]  + S1[1:9,10:18] * lambda[2] -->
<!--   b[2:10] ~ dmnorm(zero[2:10],K1)  -->
<!--   ## prior for s(coordx,coordy)...  -->
<!--   K2 <- S2[1:32,1:32] * lambda[3]  + S2[1:32,33:64] * lambda[4] -->
<!--   b[11:42] ~ dmnorm(zero[11:42],K2)  -->
<!--   ## smoothing parameter priors CHECK... -->
<!--   for (i in 1:4) { -->
<!--     lambda[i] ~ dgamma(.05,.005) -->
<!--     rho[i] <- log(lambda[i]) -->
...
=======
  ...
  ## prior for s(bathy) 
  K11 <- S1[1:9,1:9] * lambda[1, 1]  + S1[1:9,10:18] * lambda[2, 1]
  K12 <- S1[1:9,1:9] * lambda[1, 2]  + S1[1:9,10:18] * lambda[2, 2]
  K13 <- S1[1:9,1:9] * lambda[1, 3]  + S1[1:9,10:18] * lambda[2, 3]
  b1[2:10] ~ dmnorm(zero[2:10], K11) 
  b2[2:10] ~ dmnorm(zero[2:10], K12) 
  b3[2:10] ~ dmnorm(zero[2:10], K13) 

  ## prior for s(coordx,coordy) 
  K21 <- S2[1:32,1:32] * lambda[3, 1]  + S2[1:32,33:64] * lambda[4, 1]
  K22 <- S2[1:32,1:32] * lambda[3, 2]  + S2[1:32,33:64] * lambda[4, 2]
  K23 <- S2[1:32,1:32] * lambda[3, 3]  + S2[1:32,33:64] * lambda[4, 3]
  b1[11:42] ~ dmnorm(zero[11:42], K21) 
  b2[11:42] ~ dmnorm(zero[11:42], K22) 
  b3[11:42] ~ dmnorm(zero[11:42], K23) 
...
grid_bathy <- grid %>% 
  st_transform(crs = st_crs(pays)) %>%
  st_crop(xmin = 700000, xmax = 900000, ymin = 6140000, ymax = 6300000) %>%
  mutate(cov_bathy = (bathy - mbathy) / sdbathy) %>%
  pull(cov_bathy )
>>>>>>> 83c3f30226831dc8061c22df36c44a2a4efc119f
```
